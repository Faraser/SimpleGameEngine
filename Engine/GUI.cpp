...
void GUI::setWidgetDestRect(CEGUI::Window* widget, const glm::vec4& destRectPerc, const glm::vec4& destRectPix) {
    widget->setPosition(
            CEGUI::UVector2(CEGUI::UDim(destRectPerc.x, destRectPix.x), CEGUI::UDim(destRectPerc.y, destRectPix.y)));
    widget->setSize(
            CEGUI::USize(CEGUI::UDim(destRectPerc.z, destRectPix.z), CEGUI::UDim(destRectPerc.w, destRectPix.w)));
}
    ...
    switch (event.type) {
        ...
        case SDL_TEXTINPUT:
            ...
            for (int i =0; event.text.text[i] != '\0'; i++) {
                codePoint |= static_cast<CEGUI::utf32>(event.text.text[i]) << (i * 8);
            }
            m_context->injectChar(codePoint);
            break;
        case SDL_MOUSEBUTTONDOWN:
            m_context->injectMouseButtonDown(SDLButtonTOCEGUIButton(event.button.button));
            break;
        case SDL_MOUSEBUTTONUP:
            m_context->injectMouseButtonUp(SDLButtonTOCEGUIButton(event.button.button));
            break;
    }
}
